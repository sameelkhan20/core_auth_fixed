...
async def get_current_user(request: Request, creds: HTTPAuthorizationCredentials = Depends(security_scheme)):
    ...
    try:
        ...
        import warnings
        warnings.warn("Redis not available — blacklist check skipped!", RuntimeWarning)
    ...
    try:
        ...
        # Ensure only access tokens are used for authentication (Refresh token block ho jayega yahan)
